...
class Db
{
    ...
    public static function connect($config = [], $name = false)
    {
        ...
        if (true === $name || !isset(self::$instance[$name])) {
            ...
            $class = false !== strpos($options['type'], '\\') ? $options['type'] : 
            '\\system\\db\\connector\\'.ucwords($options['type']);
    ...
    public static function dbConfig(){
		return Config::getInstance()->getConfig('database');
    }
    ...
    private static function parseConfig($config)
    {
        if (empty($config)) {
            ...
            $config = Config::getInstance()->getConfig($config); // 支持读取配置参数
        }
